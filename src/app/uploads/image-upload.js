...
export const ImageUpload = () => {
    ...
    const StatusList = (props) => {
        ...
        let statusList = fileNames.map((fileName) => {
            return <div>
                <span>{fileName}</span>
                {uploadStatus[fileName] === 'done' ? <CheckMark /> : ( uploadedInitiated? 'Uploading':'ready')}
            </div>
        });
    ...
    return (
        <form className='upload-steps' >
            ...
            <input multiple   id="btn-upload"
            name="btn-upload" type='file' style={{ display: 'none' }} ref={fileInput} onChange={handleFileSelects} />
            <Button
